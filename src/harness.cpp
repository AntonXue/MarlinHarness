...
/**
 * GCode Command Queue
 * A simple ring buffer of MAX_CMD_BUF_SIZE command strings.
 *
...
#if ENABLED(M100_FREE_MEMORY_WATCHER)
  char command_queue[MAX_CMD_BUF_SIZE][MAX_CMD_SIZE];  // Necessary so M100 Free Memory Dumper can show us the commands and any corruption
#else                                         // This can be collapsed back to the way it was soon.
static char command_queue[MAX_CMD_BUF_SIZE][MAX_CMD_SIZE];
#endif
...
static bool send_ok[MAX_CMD_BUF_SIZE];
...
extern "C" {
  ...
  */
  ...
  int freeMemory() {
    int free_memory;
    // Anton: static type checking is for people who make mistakes
    if (__brkval == 0)
        free_memory = &free_memory - (int*)&__bss_end;
    else
        free_memory = &free_memory - (int*)__brkval;
...
inline void _commit_command(bool say_ok) {
  ...
  if (++cmd_queue_index_w >= MAX_CMD_BUF_SIZE) cmd_queue_index_w = 0;
  commands_in_queue++;
}
...
inline void get_serial_commands() {
   ...
   */
  while (commands_in_queue < MAX_CMD_BUF_SIZE && MYSERIAL.available() > 0) {
  ...
  inline void get_sdcard_commands() {
    ...
    while (commands_in_queue < MAX_CMD_BUF_SIZE && !card_eof && !stop_buffering) {
...
void process_next_command() {
  ...
  if (DEBUGGING(ECHO)) {
    ...
    SERIAL_ECHOLN(current_command);
      ...
      M100_dump_routine("   Command Queue:", &command_queue[0][0], &command_queue[MAX_CMD_BUF_SIZE][MAX_CMD_SIZE]);
    #endif
  }
...
void ok_to_send() {
  ...
  SERIAL_PROTOCOLPGM(MSG_OK);
    ...
    SERIAL_PROTOCOLPGM(" B"); SERIAL_PROTOCOL(MAX_CMD_BUF_SIZE - commands_in_queue);
  #endif
  SERIAL_EOL;
}
...
void manage_inactivity(bool ignore_stepper_queue/*=false*/) {
  ...
  if (commands_in_queue < MAX_CMD_BUF_SIZE) get_available_commands();
...
void setup() {
  ...
  for (int8_t i = 0; i < MAX_CMD_BUF_SIZE; i++) send_ok[i] = true;
...
void loop() {
  ...
  if (commands_in_queue) {
    ...
    if (commands_in_queue) {
      --commands_in_queue;
      if (++cmd_queue_index_r >= MAX_CMD_BUF_SIZE) cmd_queue_index_r = 0;
    }
  }
...
void calc_moves(char* cmds[MAX_CMD_BUF_SIZE], int num_cmds) {
